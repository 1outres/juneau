...
type VpcLister interface {
	...
	List(selector labels.Selector) (ret []*apiv1alpha1.Vpc, err error)
	// Get retrieves the Vpc from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*apiv1alpha1.Vpc, error)
	VpcListerExpansion
}

// vpcLister implements the VpcLister interface.
type vpcLister struct {
	listers.ResourceIndexer[*apiv1alpha1.Vpc]
}

// NewVpcLister returns a new VpcLister.
func NewVpcLister(indexer cache.Indexer) VpcLister {
	return &vpcLister{listers.New[*apiv1alpha1.Vpc](indexer, apiv1alpha1.Resource("vpc"))}
}
