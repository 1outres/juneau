...
var _ = BeforeSuite(func() {
	...
	Expect((&TransitGatewayReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)).To(Succeed())
	Expect((&TransitGatewayRouteTableReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)).To(Succeed())
	Expect((&TransitGatewayAttachmentReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr)).To(Succeed())
	...
	Expect(mgr.Add(manager.RunnableFunc(func(ctx context.Context) error {
		...
		return bootstrap.EnsureDefaults(ctx, mgr.GetClient(), logf.Log.WithName("bootstrap"), "10.16.0.0/16")
	}))).To(Succeed())
	...
	Eventually(func(g Gomega) {
		...
		// The per-Subnet IP pool is auto-generated by the
		// SubnetReconciler when it reconciles the bootstrap-created
		// default Subnet. Wait for it explicitly so individual specs
