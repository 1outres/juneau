...
ただし、noinlineが常に爆発を招くわけではありません。逆向きに効いた例が`l2_gateway`にあります。`apply_policy`をそのまま展開すると622,171命令になり、手前のNAT書き戻しが残したパケットポインタの状態の数だけルール評価を歩き直していました。全ての引数がスカラーのBPF-to-BPF subprogramに包むと132,527命令に落ちます。
